...
public class ExternalStoragePersistServiceImpl implements PersistService {
    ...
    public void removeConfigInfo4Beta(final String dataId, final String group, final String tenant) {
        ...
        tjt.execute(status -> {
            try {
                ...
                if (configInfo != null) {
                    //TODO start oracle ops 修改config_info_beta删除
                    jt.update("DELETE FROM config_info_beta WHERE data_id=? AND group_id=? AND nvl(tenant_id,0)=nvl(?,0)", dataId,
                        group, tenantTmp);
                }
            } catch (CannotGetJdbcConnectionException e) {
                LogUtil.FATAL_LOG.error("[db-error] " + e.toString(), e);
    ...
    public ConfigInfo4Beta findConfigInfo4Beta(final String dataId, final String group, final String tenant) {
        String tenantTmp = StringUtils.isBlank(tenant) ? StringUtils.EMPTY : tenant;
        try {
            //TODO start oracle ops 修改config_info_beta查询
            return this.jt.queryForObject(
                "SELECT ID,data_id,group_id,tenant_id,app_name,content,beta_ips FROM config_info_beta WHERE data_id=? AND group_id=? AND nvl(tenant_id,0)=nvl(?,0)",
    ...
    public ConfigInfo findConfigInfo(final String dataId, final String group, final String tenant) {
        final String tenantTmp = StringUtils.isBlank(tenant) ? StringUtils.EMPTY : tenant;
        try {
            //TODO start oracle ops 修改findConfigInfo查询
            return this.jt.queryForObject(
                "SELECT ID,data_id,group_id,tenant_id,app_name,content,md5,type FROM config_info WHERE data_id=? AND group_id=? AND nvl(tenant_id,0) = nvl(?,0)",
    ...
    public Page<ConfigInfo> findConfigInfo4Page(final int pageNo, final int pageSize, final String dataId,
        ...
        if (StringUtils.isNotBlank(configTags)) {
            ...
            //TODO start oracle ops修改tag分页查询
            where.append("nvl(a.tenant_id,0)=nvl(?,0)");
            //end
            ...
            //TODO start oracle ops修改分页查询
            where.append("nvl(tenant_id,0)=nvl(?,0)");
            //end
    ...
    public Page<ConfigInfoWrapper> findAllConfigInfoFragment(final long lastMaxId, final int pageSize) {
        ...
        String select = "select * from(select t.*,ROWNUM no from(SELECT id,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified,type from config_info" +
            " where id > ? order by id asc) t)where no between ? and ?";
        PaginationHelper<ConfigInfoWrapper> helper = createPaginationHelper();
        try {
    ...
    public long addConfigInfoAtomic(final long configId, final String srcIp, final String srcUser,
        ...
        try {
            ...
            try {
                //TODO start oracle ops修改自增id
                String searchSql = "select CONFIG_INFO_ID.currval from dual";
                long id = jt.queryForObject(searchSql, Long.class);
                return id;
            } catch (Exception e) {
    ...
    public List<String> selectTagByConfig(String dataId, String group, String tenant) {
        //TODO start oracle ops 修改查询标签
        String sql = "SELECT tag_name FROM config_tags_relation WHERE data_id=? AND group_id=? AND nvl(tenant_id,0) = nvl(?,0) ";
        try {
    ...
    public void removeConfigInfoAtomic(final String dataId, final String group, final String tenant, final String srcIp,
        ...
        String tenantTmp = StringUtils.isBlank(tenant) ? StringUtils.EMPTY : tenant;
        try {
            //TODO start oracle ops修改删除
            jt.update("DELETE FROM config_info WHERE data_id=? AND group_id=? AND nvl(tenant_id,0)=nvl(?,0)", dataId, group,
                tenantTmp);
        } catch (CannotGetJdbcConnectionException e) {
            LogUtil.FATAL_LOG.error("[db-error] " + e.toString(), e);
            throw e;
    ...
    public void updateConfigInfoAtomic(final ConfigInfo configInfo, final String srcIp, final String srcUser,
        ...
        try {
            //TODO start oracle ops修改更新
            jt.update("UPDATE config_info SET content=?, md5 = ?, src_ip=?,src_user=?,gmt_modified=?,"
                    + "app_name=?,c_desc=?,c_use=?,effect=?,type=?,c_schema=? "
                    + "WHERE data_id=? AND group_id=? AND nvl(TENANT_ID,0)=nvl(?,0)", configInfo.getContent(), md5Tmp, srcIp, srcUser,
    ...
    public ConfigAdvanceInfo findConfigAdvanceInfo(final String dataId, final String group, final String tenant) {
        ...
        try {
            ...
            //TODO start oracle ops 标签查询配置信息
            ConfigAdvanceInfo configAdvance = this.jt.queryForObject(
                "SELECT gmt_create,gmt_modified,src_user,src_ip,c_desc,c_use,effect,type,c_schema FROM config_info WHERE data_id=? AND group_id=? AND nvl(tenant_id,0)=nvl(?,0)",
    ...
    public ConfigAllInfo findConfigAllInfo(final String dataId, final String group, final String tenant) {
        ...
        try {
            ...
            ConfigAllInfo configAdvance = this.jt.queryForObject(
                //TODO start oracle ops修改查询findConfigAllInfo
                "SELECT ID,data_id,group_id,tenant_id,app_name,content,md5,"
                    ...
                    + "WHERE data_id=? AND group_id=? AND nvl(tenant_id,0) = nvl(?,0)", new Object[]{dataId, group, tenantTmp},
    ...
    public Page<ConfigHistoryInfo> findConfigHistory(String dataId, String group, String tenant, int pageNo,
        ...
        String tenantTmp = StringUtils.isBlank(tenant) ? StringUtils.EMPTY : tenant;
        //TODO start oracle ops 修改历史查询
        String sqlCountRows = "select count(*) from his_config_info where data_id = ? and group_id = ? and nvl(tenant_id,0) = nvl(?,0)";
        String sqlFetchRows =
            "select nid,data_id,group_id,tenant_id,app_name,src_ip,src_user,op_type,gmt_create,gmt_modified from his_config_info "
                + "where data_id = ? and group_id = ? and nvl(tenant_id,0) = nvl(?,0) order by nid desc";
