...
class ExternalStoragePaginationHelperImpl<E> implements PaginationHelper {
    ...
    public Page<E> fetchPage(final String sqlCountRows, final String sqlFetchRows, final Object[] args,
        ...
        if (isDerby()) {
            ...
            selectSql = "select * from(select a.*,ROWNUM rn from("+sqlFetchRows+ " and id > " + lastMaxId + " order by id asc "+") a where ROWNUM<=("+(startRow+pageSize)+")) where rn>"+startRow;
        } else {
            //selectSql = sqlFetchRows + " limit " + startRow + "," + pageSize;
            selectSql = "select * from(select a.*,ROWNUM rn from("+sqlFetchRows+") a where ROWNUM<="+(startRow+pageSize)+") where rn>"+startRow;
        }
